...
import { AtprotoDid, assertAtprotoDid } from "@atproto/did";
import {
  DidCache,
...
export class OAuthClient extends CustomEventTarget<OAuthClientEventMap> {
  ...
  async restore(
    ...
    const {
      ...
      authMethod = "legacy",
      tokenSet,
    } = await this.sessionGetter.get(sub as AtprotoDid, {
    ...
    try {
      ...
      if (err instanceof AuthMethodUnsatisfiableError) {
        await this.sessionGetter.delStored(sub as AtprotoDid, err);
      }
